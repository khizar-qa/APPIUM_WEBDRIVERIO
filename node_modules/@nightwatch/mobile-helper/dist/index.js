...
Object.defineProperty(exports, "AndroidSetup", { enumerable: true, get: function () { return android_1.AndroidSetup; } });
...
const run = () => {
    try {
        ...
        if (!args[0] || !constants_1.AVAILABLE_COMMANDS.includes(args[0])) {
            showHelp(args[0], options.help);
        }
        else if (args[0] === 'android') {
            const androidSetup = new android_1.AndroidSetup(options);
            androidSetup.run();
        }
        else {
...
exports.run = run;
const showHelp = (cmdPassed, helpFlag) => {
    if (cmdPassed) {
        console.log(ansi_colors_1.default.red(`unknown command: ${cmdPassed}`), '\n');
    }
    else if (!helpFlag) {
        console.log(ansi_colors_1.default.red('No command passed.'), '\n');
    }
    console.log(`Usage: ${ansi_colors_1.default.cyan('npx @nightwatch/mobile-helper COMMAND [options]')}\n`);
